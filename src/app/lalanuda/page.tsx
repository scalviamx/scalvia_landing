...
const company = {
  ...
  platformAdminEmail: "admin@miempresa.com",
  salonStaffWhatsapp: "5218112345678",
  hours: [{ day: "Todos los días", time: "10:00 – 18:00" }],
  social: { instagram: "@lalanuda.estudio", facebook: "La Lanuda Estudio" },
  businessHours: {
...
function InfoChip({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <div className="flex items-center gap-3" style={{ color: "rgba(28,24,21,0.75)" }}>
      <span style={{ color: "#C75D3A" }}>{icon}</span>
      <span>{label}</span>
    </div>
  );
}
...
function ReviewsSection({ onLeaveReview }: { onLeaveReview: () => void }) {
  ...
  return (
    <section id="resenias" style={{ backgroundColor: "#FFFEFC", borderTop: "1px solid rgba(28,24,21,0.1)", borderBottom: "1px solid rgba(28,24,21,0.1)" }}>
      <div className="max-w-6xl mx-auto px-5 md:px-8 py-20 md:py-28">
        <div className="flex items-end justify-between flex-wrap gap-6 mb-14">
          <div>
            <div className="text-xs uppercase mb-3" style={{ letterSpacing: "0.25em", color: "#8B7355" }}>Lo que dicen</div>
            <h2 className="ll-display text-4xl md:text-6xl tracking-tight max-w-2xl">Voces de la manada.</h2>
          </div>
          {totalPages > 1 && (
            <div className="flex items-center gap-2">
              <button onClick={() => setPage(p => (p - 1 + totalPages) % totalPages)} className="w-10 h-10 rounded-full flex items-center justify-center transition-colors" style={{ border: "1px solid rgba(28,24,21,0.2)" }}
                onMouseEnter={e => { e.currentTarget.style.backgroundColor = "#1C1815"; e.currentTarget.style.color = "#F5EFE6"; }}
                onMouseLeave={e => { e.currentTarget.style.backgroundColor = ""; e.currentTarget.style.color = ""; }}>
                <ChevronLeftIcon className="w-4 h-4"/>
              </button>
              <span className="text-sm px-2" style={{ color: "rgba(28,24,21,0.6)" }}>{page+1} / {totalPages}</span>
              <button onClick={() => setPage(p => (p + 1) % totalPages)} className="w-10 h-10 rounded-full flex items-center justify-center transition-colors" style={{ border: "1px solid rgba(28,24,21,0.2)" }}
                onMouseEnter={e => { e.currentTarget.style.backgroundColor = "#1C1815"; e.currentTarget.style.color = "#F5EFE6"; }}
                onMouseLeave={e => { e.currentTarget.style.backgroundColor = ""; e.currentTarget.style.color = ""; }}>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {visible.map((r, i) => (
            <article key={r.id} className={`rounded-2xl p-7 ${i===0 ? "lg:col-span-2 lg:row-span-2 lg:p-10" : ""}`}
              style={{ backgroundColor: "#F5EFE6", border: "1px solid rgba(28,24,21,0.05)" }}>
              <QuoteIcon className={`mb-4 ${i===0 ? "w-10 h-10" : "w-6 h-6"}`} style={{ color: "#C75D3A" } as React.CSSProperties}/>
              <p className={`ll-display leading-tight mb-6 ${i===0 ? "text-3xl md:text-4xl" : "text-xl"}`}>&ldquo;{r.text}&rdquo;</p>
              <div className="flex items-center justify-between mt-8 pt-5" style={{ borderTop: "1px solid rgba(28,24,21,0.1)" }}>
                <div>
                  <div className="font-medium">{r.author}</div>
                  <div className="text-xs mt-0.5" style={{ color: "rgba(28,24,21,0.55)" }}>{r.pet}</div>
                </div>
                <div className="flex gap-0.5">
                  {Array.from({length:5}).map((_,idx) => <StarIcon key={idx} className="w-3.5 h-3.5" style={{ fill: "#C75D3A", color: "#C75D3A" } as React.CSSProperties}/>)}
                </div>
              </div>
            </article>
          ))}
        ...
        <div className="mt-12 flex flex-wrap items-center justify-between gap-6">
          <div className="text-sm max-w-md" style={{ color: "rgba(28,24,21,0.6)" }}>Mostramos únicamente las reseñas de 5 estrellas verificadas tras un servicio completado.</div>
          <button onClick={onLeaveReview} className="text-sm transition-colors" style={{ textDecoration: "underline", textUnderlineOffset: "4px" }}
            onMouseEnter={e => (e.currentTarget.style.color = "#C75D3A")} onMouseLeave={e => (e.currentTarget.style.color = "")}>
...
function HomePage({ onBook, onLeaveReview }: { onBook: () => void; onLeaveReview: () => void }) {
  ...
  return (
    <main>
        ...
        <div className="max-w-6xl mx-auto px-5 md:px-8 pt-12 md:pt-24 pb-16 md:pb-28">
          <div className="grid md:grid-cols-12 gap-10 md:gap-12 items-end">
            <div className="md:col-span-7 ll-animate">
              <div className="text-xs uppercase mb-6" style={{ letterSpacing: "0.25em", color: "#8B7355" }}>Desde {company.founded} · {company.city}</div>
              <h1 className="ll-display text-5xl md:text-7xl lg:text-8xl leading-[0.95] tracking-tight">
                Cuidado <em className="not-italic font-light" style={{ color: "#C75D3A" }}>tranquilo</em>
                <br className="hidden sm:block"/> para mascotas <span className="italic font-light">queridas</span>.
              </h1>
              <p className="mt-8 text-lg md:text-xl max-w-xl leading-relaxed" style={{ color: "rgba(28,24,21,0.7)" }}>{company.description}</p>
              <div className="mt-10 flex flex-wrap gap-3">
                ...
                <a href="#galeria" className="px-7 py-4 rounded-full transition-colors" style={{ border: "1px solid rgba(28,24,21,0.2)" }}
                  onMouseEnter={e => (e.currentTarget.style.borderColor = "#1C1815")}
                  onMouseLeave={e => (e.currentTarget.style.borderColor = "rgba(28,24,21,0.2)")}>
                  Ver el estudio
                </a>
              </div>
            </div>
            <div className="md:col-span-5 relative ll-animate" style={{ animationDelay: "0.15s" }}>
              <div className="relative aspect-[4/5] rounded-3xl overflow-hidden" style={{ backgroundColor: "rgba(139,115,85,0.1)" }}>
                <img src={company.heroImage} alt="Mascota recibiendo cuidado" className="w-full h-full object-cover"/>
                <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(28,24,21,0.3), transparent)" }}/>
              </div>
              <div className="absolute -bottom-6 -left-6 md:-left-10 rounded-full w-28 h-28 md:w-36 md:h-36 flex items-center justify-center"
                style={{ backgroundColor: "#F5EFE6", border: "1px solid rgba(28,24,21,0.1)" }}>
                <div className="text-center ll-display leading-tight">
                  <div className="text-2xl md:text-3xl">{age}</div>
                  <div className="text-[10px] uppercase tracking-widest" style={{ color: "rgba(28,24,21,0.6)" }}>años</div>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Info bar */}
      <section style={{ borderTop: "1px solid rgba(28,24,21,0.1)", borderBottom: "1px solid rgba(28,24,21,0.1)", backgroundColor: "#FFFEFC" }}>
      ...
      <section id="galeria" className="max-w-6xl mx-auto px-5 md:px-8 py-20 md:py-28">
        <div className="flex items-end justify-between mb-12 gap-6 flex-wrap">
          <div>
            <div className="text-xs uppercase mb-3" style={{ letterSpacing: "0.25em", color: "#8B7355" }}>El estudio</div>
            <h2 className="ll-display text-4xl md:text-6xl tracking-tight max-w-2xl">Un ritual hecho para tu mascota.</h2>
          </div>
          <div className="text-sm max-w-xs" style={{ color: "rgba(28,24,21,0.6)" }}>Productos veterinarios premium y técnicas que respetan el bienestar animal.</div>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-5">
          {company.gallery.map((img, i) => (
            <figure key={i} className={`group relative overflow-hidden rounded-2xl ${i % 5 === 0 ? "aspect-[3/4] md:row-span-2 md:aspect-auto" : "aspect-square"}`}
              style={{ backgroundColor: "rgba(139,115,85,0.05)" }}>
              ...
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: "linear-gradient(to top, rgba(28,24,21,0.4), transparent)" }}/>
              <figcaption className="absolute bottom-3 left-3 right-3 text-xs uppercase opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: "#F5EFE6", letterSpacing: "0.1em" }}>{img.caption}</figcaption>
            </figure>
          ))}
        </div>
      </section>
      ...
      <section id="contacto" className="max-w-6xl mx-auto px-5 md:px-8 py-20 md:py-28">
        <div className="relative overflow-hidden rounded-3xl p-10 md:p-16" style={{ backgroundColor: "#1C1815", color: "#F5EFE6" }}>
          <div className="absolute top-6 right-6 opacity-15"><SparklesIcon className="w-32 h-32"/></div>
          <div className="relative max-w-2xl">
            <div className="text-xs uppercase mb-4" style={{ letterSpacing: "0.25em", color: "rgba(245,239,230,0.6)" }}>Reservas</div>
            <h2 className="ll-display text-4xl md:text-6xl leading-tight tracking-tight">Reserva en menos de un minuto.</h2>
            <p className="mt-6 text-lg leading-relaxed" style={{ color: "rgba(245,239,230,0.8)" }}>Selecciona el tipo de mascota, los servicios y paga en línea. Recibimos la confirmación de inmediato.</p>
            <button onClick={onBook} className="group mt-10 inline-flex items-center gap-2 px-7 py-4 rounded-full transition-colors"
              style={{ backgroundColor: "#F5EFE6", color: "#1C1815" }}
              onMouseEnter={e => { e.currentTarget.style.backgroundColor = "#C75D3A"; e.currentTarget.style.color = "#F5EFE6"; }}
              onMouseLeave={e => { e.currentTarget.style.backgroundColor = "#F5EFE6"; e.currentTarget.style.color = "#1C1815"; }}>
...
function PetTypeStep({ booking, patch }: { booking: Booking; patch: (u: Partial<Booking>) => void }) {
  return (
    <div>
      ...
      <p className="mb-10" style={{ color: "rgba(28,24,21,0.65)" }}>Cuéntanos del invitado especial.</p>
      <div className="grid grid-cols-2 gap-4 mb-10">
        {[{ id: "perro", Icon: DogIcon, label: "Perro" }, { id: "gato", Icon: CatIcon, label: "Gato" }].map(({ id, Icon, label }) => (
          <button key={id} onClick={() => patch({ petType: id, breed: "", sizeId: null, selectedServiceIds: [] })}
            ...
            style={{
              border: `2px solid ${booking.petType === id ? "#1C1815" : "rgba(28,24,21,0.15)"}`,
              backgroundColor: booking.petType === id ? "#1C1815" : "#FFFEFC",
              color: booking.petType === id ? "#F5EFE6" : "#1C1815",
      ...
      {booking.petType && (
        <div className="ll-animate">
          <label className="block text-xs uppercase mb-2" style={{ letterSpacing: "0.18em", color: "#8B7355" }}>Nombre de tu mascota</label>
          <input value={booking.petName} onChange={e => patch({ petName: e.target.value })} placeholder="Ej. Lupita"
            ...
            style={{ backgroundColor: "#FFFEFC", border: "1px solid rgba(28,24,21,0.1)" }}
            onFocus={e => (e.target.style.borderColor = "#1C1815")} onBlur={e => (e.target.style.borderColor = "rgba(28,24,21,0.1)")}/>
        </div>
      )}
    </div>
  );
...
function BreedSizeStep({ booking, patch }: { booking: Booking; patch: (u: Partial<Booking>) => void }) {
  ...
  return (
    <div>
      ...
      <p className="mb-10" style={{ color: "rgba(28,24,21,0.65)" }}>La raza y el tamaño nos ayudan a estimar tiempo y precio.</p>
      <div className="mb-10">
        <label className="block text-xs uppercase mb-2" style={{ letterSpacing: "0.18em", color: "#8B7355" }}>Raza</label>
        <div className="relative">
          ...
          <ChevronDownIcon className="w-5 h-5 absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: "rgba(28,24,21,0.4)" }}/>
        </div>
      </div>
      <div>
        <label className="block text-xs uppercase mb-3" style={{ letterSpacing: "0.18em", color: "#8B7355" }}>Tamaño / peso</label>
        <div className="space-y-2">
          {sizes.map(s => (
            <button key={s.id} onClick={() => patch({ sizeId: s.id })}
              ...
              style={{
                border: `2px solid ${booking.sizeId === s.id ? "#1C1815" : "rgba(28,24,21,0.15)"}`,
                backgroundColor: booking.sizeId === s.id ? "#1C1815" : "#FFFEFC",
                color: booking.sizeId === s.id ? "#F5EFE6" : "#1C1815",
              }}>
              <div>
                <div className="font-medium">{s.label}</div>
                <div className="text-xs mt-0.5" style={{ color: booking.sizeId === s.id ? "rgba(245,239,230,0.7)" : "rgba(28,24,21,0.55)" }}>{s.range}</div>
              </div>
              <div className="text-xs" style={{ color: booking.sizeId === s.id ? "rgba(245,239,230,0.7)" : "rgba(28,24,21,0.45)" }}>×{s.multiplier.toFixed(2)}</div>
            </button>
          ))}
        </div>
      </div>
...
function ServicesStep({ booking, patch }: { booking: Booking; patch: (u: Partial<Booking>) => void }) {
  ...
  return (
    <div>
      ...
      <p className="mb-10" style={{ color: "rgba(28,24,21,0.65)" }}>Puedes seleccionar más de uno. Los precios se ajustan al tamaño.</p>
        ...
        {available.map(svc => {
          ...
          return (
            <button key={svc.id} onClick={() => toggle(svc.id)}
              ...
              style={{ border: `2px solid ${isSel ? "#1C1815" : "rgba(28,24,21,0.15)"}`, backgroundColor: isSel ? "#1C1815" : "#FFFEFC", color: isSel ? "#F5EFE6" : "#1C1815" }}>
              <div className="w-6 h-6 rounded-full border-2 flex items-center justify-center mt-0.5 shrink-0 transition-colors"
                style={{ borderColor: isSel ? "#F5EFE6" : "rgba(28,24,21,0.3)", backgroundColor: isSel ? "#F5EFE6" : "transparent", color: isSel ? "#1C1815" : "transparent" }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-sm mt-1" style={{ color: isSel ? "rgba(245,239,230,0.7)" : "rgba(28,24,21,0.6)" }}>{svc.description}</div>
                <div className="text-xs mt-1" style={{ color: isSel ? "rgba(245,239,230,0.55)" : "rgba(28,24,21,0.45)" }}>≈ {svc.duration} min</div>
              </div>
            </button>
          );
        })}
...
function DateTimePicker({ durationMin, value, onChange, bookings }: {
  ...
  return (
    <div>
      <div className="text-[10px] uppercase mb-3" style={{ letterSpacing: "0.18em", color: "#8B7355" }}>Fecha</div>
          ...
          return (
            <button key={dateStr} type="button" disabled={disabled} onClick={() => setSelectedDate(d)}
              ...
              style={{
                ...
                border: `2px solid ${isSelected ? "#1C1815" : "rgba(28,24,21,0.1)"}`,
                backgroundColor: isSelected ? "#1C1815" : "#FFFEFC",
                color: isSelected ? "#F5EFE6" : "#1C1815",
                opacity: disabled ? 0.3 : 1, cursor: disabled ? "not-allowed" : "pointer",
              }}>
              <div className="text-[10px] uppercase mb-1" style={{ letterSpacing: "0.1em", color: isSelected ? "rgba(245,239,230,0.7)" : "rgba(28,24,21,0.55)" }}>{formatDayLabel(d)}</div>
              <div className="ll-display text-2xl leading-none">{d.getDate()}</div>
              <div className="text-[10px] uppercase mt-1" style={{ letterSpacing: "0.1em", color: isSelected ? "rgba(245,239,230,0.7)" : "rgba(28,24,21,0.45)" }}>{MONTH_NAMES[d.getMonth()]}</div>
            </button>
          );
        })}
      </div>
      {selectedDate && (
        <div className="mt-8 ll-animate">
              ...
              {slots.map(startMin => {
                ...
                return (
                  <button key={startMin} type="button" disabled={!available} onClick={() => onChange({ date: dateStr, startMin })}
                    ...
                    style={{
                      border: `2px solid ${isSel ? "#1C1815" : available ? "rgba(28,24,21,0.1)" : "rgba(28,24,21,0.05)"}`,
                      backgroundColor: isSel ? "#1C1815" : available ? "#FFFEFC" : "rgba(28,24,21,0.05)",
                      color: isSel ? "#F5EFE6" : available ? "#1C1815" : "rgba(28,24,21,0.3)",
          ...
          {slots.length > 0 && (
            <div className="text-[11px] mt-3 flex items-center gap-1.5" style={{ color: "rgba(28,24,21,0.45)" }}>
      ...
      {value && (
        <div className="mt-6 rounded-2xl p-4 text-sm flex items-start gap-3 ll-animate" style={{ backgroundColor: "#1C1815", color: "#F5EFE6" }}>
          <CheckIcon className="w-4 h-4 shrink-0 mt-0.5" style={{ color: "#C75D3A" } as React.CSSProperties}/>
          <div>
            <div className="font-medium">{formatLongDate(value.date)} a las {minToHHMM(value.startMin)}</div>
            <div className="text-xs mt-0.5" style={{ color: "rgba(245,239,230,0.65)" }}>Termina aprox. {minToHHMM(value.startMin + durationMin)} · {durationMin} min</div>
          </div>
        </div>
      )}
    </div>
...
function SummaryStep({ booking, patch, user, onConfirmed }: {
  ...
  if (stage === "confirmed") return (
    <div className="text-center py-12 ll-animate">
      <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6" style={{ backgroundColor: "#1C1815", color: "#F5EFE6" }}>
      ...
      <div className="rounded-2xl p-5 max-w-md mx-auto mb-8 text-left" style={{ backgroundColor: "#FFFEFC", border: "1px solid rgba(28,24,21,0.1)" }}>
        <div className="text-[10px] uppercase mb-2" style={{ letterSpacing: "0.1em", color: "rgba(28,24,21,0.55)" }}>Tu cita</div>
        <div className="ll-display text-xl leading-tight">{dateLabel}</div>
        <div className="text-sm mt-1" style={{ color: "rgba(28,24,21,0.6)" }}>{booking.petName} · {items.length} {items.length === 1 ? "servicio" : "servicios"} · {formatMXN(total)}</div>
      </div>
      <button onClick={onConfirmed} className="px-7 py-3.5 rounded-full transition-colors" style={{ backgroundColor: "#1C1815", color: "#F5EFE6" }}
        onMouseEnter={e => (e.currentTarget.style.backgroundColor = "#C75D3A")} onMouseLeave={e => (e.currentTarget.style.backgroundColor = "#1C1815")}>
        Volver al inicio
      </button>
    </div>
  );
  ...
  return (
    <div>
      ...
      <p className="mb-10" style={{ color: "rgba(28,24,21,0.65)" }}>Elige cuándo quieres venir, cómo pagas, y confirmamos.</p>
      <div className="rounded-3xl p-6 md:p-8 space-y-5" style={{ backgroundColor: "#FFFEFC", border: "1px solid rgba(28,24,21,0.1)" }}>
        {[["Mascota", `${booking.petName} · ${booking.petType === "perro" ? "Perro" : "Gato"} · ${booking.breed}`], ["Tamaño", `${size?.label} (${size?.range})`]].map(([k, v]) => (
          <div key={k} className="flex justify-between gap-4 items-baseline">
            <span className="text-[10px] uppercase shrink-0 pt-1" style={{ letterSpacing: "0.1em", color: "rgba(28,24,21,0.55)" }}>{k}</span>
            <span className="text-right">{v}</span>
          </div>
        ))}
        <div>
          <div className="flex justify-between items-baseline mb-3">
            <span className="text-[10px] uppercase" style={{ letterSpacing: "0.1em", color: "rgba(28,24,21,0.55)" }}>Servicios</span>
            <span className="text-[10px] uppercase" style={{ letterSpacing: "0.1em", color: "rgba(28,24,21,0.55)" }}>~{durationMin} min</span>
          </div>
          <div className="space-y-2">
            {items.map(it => (
              <div key={it.id} className="flex justify-between gap-3 pb-2" style={{ borderBottom: "1px solid rgba(28,24,21,0.1)" }}>
                <span>{it.name} <span className="text-xs ml-1" style={{ color: "rgba(28,24,21,0.45)" }}>{it.duration} min</span></span>
                <span>{formatMXN(it.price)}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="flex justify-between items-baseline pt-4" style={{ borderTop: "1px solid rgba(28,24,21,0.15)" }}>
          <span className="text-xs uppercase" style={{ letterSpacing: "0.1em", color: "rgba(28,24,21,0.55)" }}>Total</span>
      ...
      <div className="mt-10">
        <div className="flex items-baseline justify-between mb-5 flex-wrap gap-2">
          <h3 className="ll-display text-2xl tracking-tight flex items-center gap-2">
            <CalendarIcon className="w-5 h-5" style={{ color: "#C75D3A" } as React.CSSProperties}/> Fecha y hora
          </h3>
          <span className="text-xs" style={{ color: "rgba(28,24,21,0.55)" }}>Reservas con 24h de anticipación</span>
        </div>
        <DateTimePicker durationMin={durationMin} value={booking.slot} onChange={slot => patch({ slot })} bookings={existingBookings}/>
      </div>
      <div className="mt-8">
        <label className="block text-xs uppercase mb-2" style={{ letterSpacing: "0.18em", color: "#8B7355" }}>Notas para el estilista (opcional)</label>
      ...
      <div className="mt-10">
        <h3 className="ll-display text-2xl tracking-tight flex items-center gap-2 mb-5">
          <CreditCardIcon className="w-5 h-5" style={{ color: "#C75D3A" } as React.CSSProperties}/> Método de pago
        </h3>
        <div className="space-y-2">
          {PAYMENT_METHODS.map(pm => {
            ...
            return (
              <button key={pm.id} type="button" disabled={locked} onClick={() => patch({ paymentMethod: pm.id })}
                ...
                style={{ border: `2px solid ${isSel ? "#1C1815" : "rgba(28,24,21,0.1)"}`, backgroundColor: isSel ? "#1C1815" : "#FFFEFC", color: isSel ? "#F5EFE6" : "#1C1815", opacity: locked ? 0.6 : 1, cursor: locked ? "not-allowed" : "pointer" }}>
                <div className="w-6 h-6 rounded-full border-2 flex items-center justify-center mt-0.5 shrink-0"
                  style={{ borderColor: isSel ? "#F5EFE6" : "rgba(28,24,21,0.3)", backgroundColor: isSel ? "#F5EFE6" : "transparent", color: isSel ? "#1C1815" : "transparent" }}>
                  {isSel && <CheckIcon className="w-3.5 h-3.5" strokeWidth={3}/>}
                </div>
                <PmIcon className="w-5 h-5 mt-0.5 shrink-0" style={{ color: isSel ? "#F5EFE6" : "#C75D3A" } as React.CSSProperties}/>
                <div className="flex-1 min-w-0">
                  <div className="font-medium">{pm.title}</div>
                  <div className="text-sm mt-0.5" style={{ color: isSel ? "rgba(245,239,230,0.7)" : "rgba(28,24,21,0.6)" }}>{pm.desc}</div>
                </div>
              </button>
            );
          })}
      ...
      {stage === "review" && (() => {
        if (!booking.slot || !booking.paymentMethod) return (
          <button disabled className="mt-10 w-full py-4 rounded-full inline-flex items-center justify-center gap-2" style={{ backgroundColor: "#1C1815", color: "#F5EFE6", opacity: 0.3, cursor: "not-allowed" }}>
            {!booking.slot ? "Selecciona fecha y hora" : "Selecciona método de pago"}
          </button>
        );
        if (booking.paymentMethod === "online") return (
          <button onClick={handlePay} className="mt-10 w-full py-4 rounded-full inline-flex items-center justify-center gap-2 transition-colors" style={{ backgroundColor: "#1C1815", color: "#F5EFE6" }}
            onMouseEnter={e => (e.currentTarget.style.backgroundColor = "#C75D3A")} onMouseLeave={e => (e.currentTarget.style.backgroundColor = "#1C1815")}>
            <CreditCardIcon className="w-4 h-4"/> Pagar {formatMXN(total)} en línea
          </button>
        );
        return (
          <button onClick={handleConfirm} className="mt-10 w-full py-4 rounded-full inline-flex items-center justify-center gap-2 transition-colors" style={{ backgroundColor: "#1C1815", color: "#F5EFE6" }}
            onMouseEnter={e => (e.currentTarget.style.backgroundColor = "#C75D3A")} onMouseLeave={e => (e.currentTarget.style.backgroundColor = "#1C1815")}>
      ...
      {stage === "paying" && (
        <button disabled className="mt-10 w-full py-4 rounded-full inline-flex items-center justify-center gap-2" style={{ backgroundColor: "rgba(28,24,21,0.6)", color: "#F5EFE6", cursor: "wait" }}>
      ...
      {stage === "paid" && (
        <div className="mt-10 space-y-4 ll-animate">
          <div className="rounded-2xl p-5 flex items-start gap-3" style={{ backgroundColor: "rgba(28,24,21,0.05)" }}>
            <CheckIcon className="w-5 h-5 shrink-0 mt-0.5" style={{ color: "#C75D3A" } as React.CSSProperties}/>
            <div className="text-sm">
              <div className="font-medium">Pago recibido (simulado)</div>
              <div className="text-xs mt-0.5" style={{ color: "rgba(28,24,21,0.6)" }}>En producción esto se valida con el webhook de MercadoPago / Stripe.</div>
            </div>
          </div>
          <button onClick={handleConfirm} className="w-full py-4 rounded-full inline-flex items-center justify-center gap-2 transition-colors" style={{ backgroundColor: "#1C1815", color: "#F5EFE6" }}
            onMouseEnter={e => (e.currentTarget.style.backgroundColor = "#C75D3A")} onMouseLeave={e => (e.currentTarget.style.backgroundColor = "#1C1815")}>
            <MessageCircleIcon className="w-4 h-4"/> Confirmar y notificar al equipo
          </button>
          <p className="text-xs text-center max-w-sm mx-auto" style={{ color: "rgba(28,24,21,0.5)" }}>Se abrirá WhatsApp (para el salón) y tu cliente de correo (para el administrador).</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <main className="max-w-3xl mx-auto px-5 md:px-8 py-8 md:py-12 pb-32">
      <div className="flex items-center gap-2 mb-8">
        <button onClick={back} className="p-2 -ml-2 rounded-full transition-colors" onMouseEnter={e => (e.currentTarget.style.backgroundColor = "rgba(28,24,21,0.05)")} onMouseLeave={e => (e.currentTarget.style.backgroundColor = "")}>
          <ArrowLeftIcon className="w-5 h-5"/>
        </button>
        <button onClick={onCancel} className="ml-auto p-2 -mr-2 rounded-full transition-colors" onMouseEnter={e => (e.currentTarget.style.backgroundColor = "rgba(28,24,21,0.05)")} onMouseLeave={e => (e.currentTarget.style.backgroundColor = "")}>
          <XIcon className="w-5 h-5"/>
        </button>
      </div>
      <div className="flex items-center gap-1.5 mb-3">
        {STEPS.map((s, i) => <div key={s.id} className="h-1 flex-1 rounded-full transition-colors duration-500" style={{ backgroundColor: i <= stepIdx ? "#1C1815" : "rgba(28,24,21,0.15)" }}/>)}
      </div>
      <div className="text-xs uppercase mb-10" style={{ letterSpacing: "0.25em", color: "#8B7355" }}>Paso {stepIdx + 1} de {STEPS.length} · {STEPS[stepIdx].label}</div>
      ...
      {!isLast && (
        <div className="fixed bottom-0 left-0 right-0 z-20 backdrop-blur-md" style={{ backgroundColor: "rgba(245,239,230,0.95)", borderTop: "1px solid rgba(28,24,21,0.1)" }}>
          <div className="max-w-3xl mx-auto px-5 md:px-8 py-4 flex items-center justify-between">
            <div>
              <div className="text-[10px] uppercase" style={{ letterSpacing: "0.1em", color: "rgba(28,24,21,0.55)" }}>Total estimado{duration > 0 ? ` · ~${duration} min` : ""}</div>
            ...
            <button disabled={!canAdvance} onClick={() => setStepIdx(i => i + 1)}
              ...
              style={{ backgroundColor: "#1C1815", color: "#F5EFE6", opacity: canAdvance ? 1 : 0.3, cursor: canAdvance ? "pointer" : "not-allowed" }}
              onMouseEnter={e => canAdvance && (e.currentTarget.style.backgroundColor = "#C75D3A")}
              onMouseLeave={e => (e.currentTarget.style.backgroundColor = "#1C1815")}>
...
function ReviewSubmit({ user, onDone }: { user: { name?: string } | null; onDone: () => void }) {
  ...
  if (submitted) {
    ...
    return (
      <main className="max-w-2xl mx-auto px-5 md:px-8 py-20 text-center ll-animate">
        <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6" style={{ backgroundColor: "#1C1815", color: "#F5EFE6" }}><CheckIcon className="w-10 h-10" strokeWidth={2.4}/></div>
        <h2 className="ll-display text-4xl tracking-tight mb-4">¡Gracias por tu opinión!</h2>
        <p className="mb-10 max-w-md mx-auto leading-relaxed" style={{ color: "rgba(28,24,21,0.65)" }}>
        ...
        <button onClick={onDone} className="px-7 py-3.5 rounded-full transition-colors" style={{ backgroundColor: "#1C1815", color: "#F5EFE6" }}
          onMouseEnter={e => (e.currentTarget.style.backgroundColor = "#C75D3A")} onMouseLeave={e => (e.currentTarget.style.backgroundColor = "#1C1815")}>
          Volver al inicio
        </button>
      </main>
  ...
  return (
    <main className="max-w-2xl mx-auto px-5 md:px-8 py-16 md:py-24">
      <div className="text-xs uppercase mb-3" style={{ letterSpacing: "0.25em", color: "#8B7355" }}>Tu opinión</div>
      <h2 className="ll-display text-4xl md:text-5xl tracking-tight mb-4">¿Cómo estuvo tu visita?</h2>
      <p className="mb-12 leading-relaxed" style={{ color: "rgba(28,24,21,0.65)" }}>Las reseñas de 5 estrellas se muestran en nuestro muro público. Las demás llegan privadas al equipo.</p>
      <div className="mb-10">
        <label className="block text-xs uppercase mb-4" style={{ letterSpacing: "0.18em", color: "#8B7355" }}>Calificación</label>
          ...
          {[1,2,3,4,5].map(n => (
            <button key={n} onMouseEnter={() => setHover(n)} onMouseLeave={() => setHover(0)} onClick={() => setRating(n)} className="p-1">
              <StarIcon className={`w-10 h-10 transition-all ${n <= (hover || rating) ? "scale-110" : ""}`}
                style={{ fill: n <= (hover || rating) ? "#C75D3A" : "transparent", color: n <= (hover || rating) ? "#C75D3A" : "rgba(28,24,21,0.2)" } as React.CSSProperties}/>
            </button>
          ))}
        </div>
      </div>
      <div className="mb-6">
        ...
        <input value={petName} onChange={e => setPetName(e.target.value)} placeholder="Ej. Bruno, French Bulldog"
          className="w-full rounded-2xl px-5 py-4 focus:outline-none" style={{ backgroundColor: "#FFFEFC", border: "1px solid rgba(28,24,21,0.1)" }}
          onFocus={e => (e.target.style.borderColor = "#1C1815")} onBlur={e => (e.target.style.borderColor = "rgba(28,24,21,0.1)")}/>
      </div>
      <div className="mb-10">
        <label className="block text-xs uppercase mb-2" style={{ letterSpacing: "0.18em", color: "#8B7355" }}>Comentario</label>
        <textarea rows={5} value={text} onChange={e => setText(e.target.value)} placeholder="Cuéntanos cómo fue la experiencia..."
          className="w-full rounded-2xl px-5 py-4 focus:outline-none resize-none" style={{ backgroundColor: "#FFFEFC", border: "1px solid rgba(28,24,21,0.1)" }}
          onFocus={e => (e.target.style.borderColor = "#1C1815")} onBlur={e => (e.target.style.borderColor = "rgba(28,24,21,0.1)")}/>
      </div>
      <button onClick={() => setSubmitted(true)} disabled={!rating || !text.trim()}
        ...
        style={{ backgroundColor: "#1C1815", color: "#F5EFE6", opacity: (!rating || !text.trim()) ? 0.3 : 1, cursor: (!rating || !text.trim()) ? "not-allowed" : "pointer" }}
        onMouseEnter={e => (rating && text.trim()) && (e.currentTarget.style.backgroundColor = "#C75D3A")}
        onMouseLeave={e => (e.currentTarget.style.backgroundColor = "#1C1815")}>
        Enviar reseña
      </button>
    </main>
...
function Footer() {
  return (
    <footer style={{ backgroundColor: "#1C1815", color: "#F5EFE6" }}>
      <div className="max-w-6xl mx-auto px-5 md:px-8 py-16 md:py-20 grid md:grid-cols-12 gap-10">
        <div className="md:col-span-5">
          ...
          <div className="text-sm mb-6" style={{ color: "rgba(245,239,230,0.6)" }}>{company.tagline}</div>
          <p className="text-sm leading-relaxed max-w-sm" style={{ color: "rgba(245,239,230,0.7)" }}>{company.description}</p>
        </div>
        <div className="md:col-span-3 md:col-start-7">
          <div className="text-xs uppercase mb-4" style={{ letterSpacing: "0.1em", color: "rgba(245,239,230,0.5)" }}>Visítanos</div>
          <ul className="space-y-3 text-sm" style={{ color: "rgba(245,239,230,0.85)" }}>
            <li className="flex items-start gap-2"><MapPinIcon className="w-4 h-4 mt-0.5 shrink-0" style={{ color: "#C75D3A" } as React.CSSProperties}/><span>{company.address}</span></li>
            {company.hours.map(h => <li key={h.day} className="flex justify-between gap-2"><span>{h.day}</span><span style={{ color: "rgba(245,239,230,0.55)" }}>{h.time}</span></li>)}
          </ul>
        </div>
        <div className="md:col-span-3">
          <div className="text-xs uppercase mb-4" style={{ letterSpacing: "0.1em", color: "rgba(245,239,230,0.5)" }}>Contacto</div>
          <ul className="space-y-3 text-sm" style={{ color: "rgba(245,239,230,0.85)" }}>
            <li className="flex items-center gap-2"><PhoneIcon className="w-4 h-4 shrink-0" style={{ color: "#C75D3A" } as React.CSSProperties}/><span>{company.phone}</span></li>
            <li className="flex items-center gap-2"><MailIcon className="w-4 h-4 shrink-0" style={{ color: "#C75D3A" } as React.CSSProperties}/><span>{company.email}</span></li>
            <li className="flex items-center gap-2"><InstagramIcon className="w-4 h-4 shrink-0" style={{ color: "#C75D3A" } as React.CSSProperties}/><span>{company.social.instagram}</span></li>
            <li className="flex items-center gap-2"><FacebookIcon className="w-4 h-4 shrink-0" style={{ color: "#C75D3A" } as React.CSSProperties}/><span>{company.social.facebook}</span></li>
          </ul>
        </div>
      </div>
      <div style={{ borderTop: "1px solid rgba(245,239,230,0.1)" }}>
        <div className="max-w-6xl mx-auto px-5 md:px-8 py-5 flex flex-wrap justify-between gap-3 text-xs" style={{ color: "rgba(245,239,230,0.5)" }}>
...
export default function LalanudaPage() {
  ...
  if (!isLoaded) {
    return (
      <div className="flex items-center justify-center min-h-screen text-xl" style={{ backgroundColor: "#F5EFE6", color: "#8B7355", fontFamily: "'Fraunces', serif" }}>
        Cargando La Lanuda...
      </div>
    );
  }
